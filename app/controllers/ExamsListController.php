...
class ExamsListController extends \BaseController {
	...
	function fetchMarks(){
		...
		$toReturn['exam'] = examsList::where('id',Input::get('exam'))->first()->toArray();
		...
		$examMarks = examMarks::where('examId',Input::get('exam'))->where('classId',Input::get('classId'))->where('subjectId',Input::get('subjectId'))->get();
