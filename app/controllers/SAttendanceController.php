...
class SAttendanceController extends \BaseController {
	...
	public function __construct(){
		...
		if(!$this->data['users']->hasThePerm('staffAttendance')){
			exit;
		}
	}
	...
	public function listAttendance(){
		...
		$vacationList = vacation::where('vacDate',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->where('acYear',$this->panelInit->selectAcYear)->where('role','teacher')->get();

		$attendanceArray = attendance::where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->get();
			...
			if(isset($value['attendance']) AND strlen($value['attendance']) > 0){
				if(!isset($attendanceList[$key])){
					...
					if($attendanceList[$key] != $value['attendance']){
						$attendanceN = attendance::where('studentId',$key)->where('date',$this->panelInit->dateToUnix(Input::get('attendanceDay')))->first();
						$attendanceN->status = $value['attendance'];
