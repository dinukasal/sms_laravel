		...
		if(count($promote) > 0){
			...
			foreach ($users as $value) {
				if(studentAcademicYears::where('studentId',$value->id)->where('academicYearId',$promote[$value->id]['acYear'])->count() > 0){
					...
					User::where('id',$value->id)->update(array('studentSection'=>$promote[$value->id]['section'],'studentClass'=>$promote[$value->id]['class'],'studentAcademicYear'=>$promote[$value->id]['acYear']));
