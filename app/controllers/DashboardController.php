...
class DashboardController extends \BaseController {
	...
	public function calender()
	{
		...
		$examsList = examsList::where('examDate','>',$daysArray['start'])->where('examDate','<',$daysArray['end'])->get();
		...
		if($this->data['users']->role == "admin"){
			$onlineExams = onlineExams::where('examDate','>',$daysArray['start'])->where('examDate','<',$daysArray['end'])->get();
		}elseif($this->data['users']->role == "teacher"){
			$onlineExams = onlineExams::where('examDate','>',$daysArray['start'])->where('examDate','<',$daysArray['end'])->where('examTeacher',$this->data['users']->id)->get();
		}elseif($this->data['users']->role == "student"){
			$onlineExams = onlineExams::where('examDate','>',$daysArray['start'])->where('examDate','<',$daysArray['end'])->where('examClass','like','%"'.$this->data['users']->studentClass.'"%')->get();
		}
	...
	public function mobNotif($id = ""){
		...
		if($this->data['users']->role == "admin"){
			$mobNotifications = mobNotifications::where('notifTo','users')->where('notifToIds','LIKE','%"'.$userId.'"%')->orWhere('notifTo','all');
		}
		if($this->data['users']->role == "teacher"){
			$mobNotifications = mobNotifications::where('notifTo','teachers')->orWhere(function($query) use($userId) {
																						        $query->where('notifTo', 'users')
																						              ->where('notifToIds','LIKE','%"'.$userId.'"%');
																						})->orWhere('notifTo','all');
		}
		if($this->data['users']->role == "parent"){
			$mobNotifications = mobNotifications::where('notifTo','parents')->orWhere(function($query) use($userId) {
																						        $query->where('notifTo', 'users')
																						              ->where('notifToIds','LIKE','%"'.$userId.'"%');
																						})->orWhere('notifTo','all');
		}
		if($this->data['users']->role == "studnet"){
			...
			$mobNotifications = mobNotifications::where(function($query) use($userClass) {
