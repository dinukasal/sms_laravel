...
class MailSmsHandler {
	...
	public function concepto($to,$message){
		...
		$to = $this->prepareNumberFormat($to,"00");
		$sess = file_get_contents("http://premium.liveair.in/httpapi/smsapi?uname=".$this->settings['conceptoUserName']."&password=".$this->settings['conceptoPassword']."&sender=".$this->settings['conceptoSenderId']."&receiver=".$to."&route=T&msgtype=1&sms=".urlencode($message));
		if(Config::get('app.debug') == true){
			Log::info($sess);
		}
	}
	...
	public function enterpriseliveair($to,$message){
		...
		file_get_contents("http://enterprise.liveair.in/httpapi/smsapi?uname=".$this->settings['enterpriseliveairUserName']."&password=".$this->settings['enterpriseliveairPassword']."&sender=".$this->settings['enterpriseliveairSenderId']."&receiver=".$to."&route=2&msgtype=1&sms=".urlencode($message));
	}
